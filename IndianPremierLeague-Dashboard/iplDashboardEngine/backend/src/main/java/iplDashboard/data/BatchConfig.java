...
public class BatchConfig {
    ...
    @Bean
    public FlatFileItemReader<OfficialMatch> reader() {
        return new FlatFileItemReaderBuilder<OfficialMatch>().name("MatchItemReader")
                .resource(new ClassPathResource("match-data.csv")).delimited().names(FIELD_NAMES)
                .fieldSetMapper(new BeanWrapperFieldSetMapper<OfficialMatch>() {
                    {
                        setTargetType(OfficialMatch.class);
                    }
                }).build();
    }
